...
| Direct target-anchor provenance validation | `analysis_tools/validate_target_anchor_evidence.py` |
...
| Fail-closed analysis view for invalid direct target-anchor evidence | `analysis_tools/build_fail_closed_analysis_view.py` |
...
| Full-resolution eye-constrained ARAP case execution | `revision_tools/run_arap_full_resolution_case.py` |
| Sequential full-resolution ARAP batch execution | `revision_tools/run_arap_full_resolution_batch.py` |
| Development-only ARAP setting selection | `analysis_tools/select_arap_full_resolution_development.py` |
| Paired S8--ARAP continuous analysis | `analysis_tools/analyze_arap_full_resolution.py` |
| All-attempt S8 component ablation | `revision_tools/run_s8_component_ablation.py` |
...
The current ARAP comparison uses the original HRN source topology, direct
full-resolution deformation, all eye/orbit vertices as positional constraints,
and no displacement transfer or post-solve eye restoration.
