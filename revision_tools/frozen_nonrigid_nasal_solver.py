...
ROOT = Path(os.environ.get("PAPERFIT_ROOT", ".")).expanduser().resolve()
MANIFEST = ROOT / "prepared_cohort" / "facescape_frontal_pairs_manifest.csv"
RIGID_ROOT = ROOT / "rigid_outputs"
OUT = ROOT / "nonrigid_outputs"
CASES = []
...
def build_masks(obj_path, vertices):
    ...
    subnasal = valid_uv & uv_ellipse(px, py, 0.500 * w, 0.635 * h, 0.070 * w, 0.030 * h)
